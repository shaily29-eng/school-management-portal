...
describe('Utility Functions Tests', () => {
  ...
  test('calculateOverallAttendancePercentage', () => {
    ...
    // Calculate the expected percentage rounded to two decimal places
    const expectedRoundedPercentage = Number(expectedPercentage.toFixed(2));
    const calculatedPercentage = calculateOverallAttendancePercentage(subjectAttendance);
    // Round the calculated percentage to two decimal places for comparison
    const calculatedRoundedPercentage = Number(calculatedPercentage.toFixed(2));
